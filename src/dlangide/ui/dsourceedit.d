...
/// Header widget for DSourceEdit
class EditorHeader : HorizontalLayout {
    private TextWidget _filenameText;
    private TextWidget _pathText;
    private ImageWidget _modifiedIcon;
    ...
    this(string filename) {
        ...
        // Modified indicator
        _modifiedIcon = new ImageWidget("modified_icon", "text-plain");
        _modifiedIcon.layoutWidth(16).layoutHeight(16);
        _modifiedIcon.visibility = Visibility.Gone;
        addChild(_modifiedIcon);
    }
    
    void setModified(bool modified) {
        _modifiedIcon.visibility = modified ? Visibility.Visible : Visibility.Gone;
    }
}

/// Wrapper for DSourceEdit with a header
class EditorWithHeader : VerticalLayout {
    private DSourceEdit _editor;
    private EditorHeader _header;
    ...
    this(string filename) {
        ...
        _editor = new DSourceEdit(filename);
        addChild(_header);
        addChild(_editor);
        _editor.modifiedStateChange = (Widget source, bool modified) {
            _header.setModified(modified);
        };
    }
    
    @property DSourceEdit editor() { return _editor; }
    @property EditorHeader header() { return _header; }
