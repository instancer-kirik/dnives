...
class InlineChangeWidget : HorizontalLayout {
    ...
    // Event handlers as delegate properties
    bool delegate(InlineChange, bool) onChangeAction; // called with (change, accepted)
    bool delegate(InlineChange) onPreviewRequested;
...
class InlineDiffEditor : VerticalLayout {
    ...
    void saveFile(string filePath = null) {
        ...
        try {
            string content = std.utf.toUTF8(_editor.text);
    ...
    void showInlineSuggestions(InlineChange[] changes) {
        ...
        foreach (change; changes) {
            ...
            widget.onChangeAction = delegate(InlineChange ch, bool accepted) {
    ...
    @property string content() {
        string content = _editor ? std.utf.toUTF8(_editor.text) : "";
        return content;
    }
    ...
    @property void content(string value) {
        if (_editor) {
            _editor.text = std.utf.toUTF32(value);
            _hasUnsavedChanges = true;
        }
    }
    ...
    void compareWithFile(string otherFilePath) {
        try {
            ...
            string otherContent = std.utf.toUTF8(readText(otherFilePath));
            string currentContent = std.utf.toUTF8(_editor.text);
