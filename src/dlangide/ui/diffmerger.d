...
class DiffMergerWidget : Dialog {
    ...
    this(string originalText = "", string suggestedText = "", string filePath = null) {
        super(UIString.fromRaw("Diff Merger"), null,
    ...
    private Widget createDiffsSection() {
        ...
        _diffsLayout = new VerticalLayout("diffsLayout");
    ...
    private void setupEventHandlers() {
        ...
        // Note: Scroll synchronization disabled - SourceEdit doesn't have onScrollPosChange
        // This would need alternative implementation using scroll event handling
    }
    ...
    private void showDiff() {
        string originalText = std.utf.toUTF8(_originalEditor.text);
        string suggestedText = std.utf.toUTF8(_suggestedEditor.text);

        if (originalText.empty && suggestedText.empty) {
            // Use window's showMessageBox method instead of global function
            if (window) {
                window.showMessageBox("Warning"d, "Both editors are empty"d);
            }
            return;
        }
            ...
            foreach (symbol; _config.keySymbols) {
                if (trimmedLine.startsWith(symbol ~ " ") ||
                    ...
                    (trimmedLine.length > symbol.length &&
                     trimmedLine.startsWith(symbol) &&
                     !isAlphaNum(trimmedLine[symbol.length]))) {
                    isKeyLine = true;
                    break;
                }
            }
    ...
    private void updateDiffDisplay() {
        ...
        foreach (i, ref block; _diffBlocks) {
            Widget diffWidget = createDiffBlockWidget(cast(int)i, block);
    ...
    string getMergedContent() {
        if (_resultEditor) {
            return std.utf.toUTF8(_resultEditor.text);
        }
        return "";
    }
    ...
    override void close(const Action action) {
        if (action.id == StandardAction.Ok) {
            if (hasUnresolvedConflicts()) {
                if (window) {
                    window.showMessageBox("Unresolved Conflicts"d,
                                 "There are still unresolved conflicts. Please resolve them first."d);
                }
                return;
            }

            // Emit resolved event - signal takes no args
