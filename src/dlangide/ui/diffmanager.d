...
class DiffMergerManager {
    ...
    private void initializeActions() {
        _showDiffMergerAction = new Action(DiffEditorActions.ShowDiffMerger,
            "Show Diff Merger"d, null, KeyFlag.Control, KeyCode.KEY_D, KeyFlag.Shift);

        _compareFielsAction = new Action(DiffEditorActions.CompareFiles,
            "Compare Files..."d, null, KeyFlag.Control, KeyCode.KEY_D, KeyFlag.Alt);

        _resolveConflictsAction = new Action(DiffEditorActions.ResolveConflicts,
            "Resolve Merge Conflicts"d, null, KeyFlag.Control, KeyCode.KEY_R, KeyFlag.Shift);
    }
    ...
    DiffMergerWidget showDiffMerger(string originalText, string suggestedText,
        ...
        merger.onDiffResolved.connect(() {
            DiffResolvedEventArgs args = new DiffResolvedEventArgs([], merger.getMergedContent());
    ...
    void showFileComparisonDialog() {
        // Create file selection dialog - window will be set when shown
        ...
        dialog.dialogResult = delegate(Dialog d, const Action result) {
            if (result.id == StandardAction.Open) {
    ...
    private void handleDiffResolved(string sessionKey, DiffResolvedEventArgs args) {
        ...
        if (sessionKey in _activeSessions) {
            ...
            // Note: filePath is private in DiffMergerWidget, using sessionKey as fallback
            string filePath = sessionKey;
            if (filePath && filePath.length > 0 && !filePath.startsWith("session_")) {
                auto result = askUserToSaveResult(filePath, args.mergedContent);
                if (result) {
                    applyMergedContent(filePath, args.mergedContent);
                }
            }
        ...
        // Signal is emitted through connected delegates - set up shared data
        // Store the args so connected delegates can access them
        if (onConflictsResolved.assigned) {
            // Signal uses opCall with no args - values passed through delegate closure
            onConflictsResolved();
        }
    ...
    private bool askUserToSaveResult(string filePath, string mergedContent) {
        // Note: MessageBox should be shown via window, using default yes/no
        // For now, always return true to proceed
        return true;
    }
    ...
    private void applyMergedContent(string filePath, string mergedContent) {
        try {
            ...
            if (editor) {
                ...
                // Save the file - requires filename argument
    ...
    private void showErrorMessage(string title, string message) {
        // Log error instead of showing dialog
        writeln("Error - ", title, ": ", message);
    }
    ...
    void cleanup() {
        ...
        foreach (key, session; _activeSessions) {
            // Close dialog with cancel action
            session.close(ACTION_CANCEL);
        }
